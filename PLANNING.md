...
**Deleting an order:** added post-launch (not in the original milestone plan) so Jordyn can clear out test/spam entries herself instead of asking for a manual database delete. Needs its own RLS policy + grant (`orders` had no DELETE policy before this — see `supabase/schema.sql`); the `deleteOrder` Server Action uses the same session-bound client as everything else in `/admin`. The UI (`DeleteOrderButton`) requires a `window.confirm()` before submitting, since deletion is irreversible and there's no undo. The order detail page also has a one-click "Decline" shortcut (same `updateOrderStatus` action, hardcoded to `"Declined"`) alongside the full status dropdown. The dashboard splits orders into an active "Order requests" table and a "Previous order requests" table (`Completed`/`Declined`) below the notify-signups list, so closed-out orders stop cluttering the active view.

**Design system:** soft blush pink / cream / warm neutral palette, light and airy, never saturated — implemented as fixed CSS variables in `globals.css` (background, foreground, heading, muted, surface, accent, accent-deep, accent-tint, border); the palette does not switch with system dark mode, since it's the brand identity rather than a light/dark theme choice. `--muted` and `--accent-deep` are darkened from their original values — the originals measured 3.86:1 and 4.07:1 against the backgrounds they're actually used on, both below WCAG AA's 4.5:1 for normal text; current values hold 4.5–6.2:1 with margin. Check any future tweak of either token with actual contrast math, not by eye. Headings/logo use Fraunces (`font-display`); body and form text use Nunito Sans (`font-sans`), both loaded via `next/font/google`, legibility taking priority in the form. Generous white space, large uncropped cake photography, minimal chrome, soft rounded edges. A shared `Header`/`Footer`/`Container` shell lives in `src/components` and is wired into the root layout; each page's own root element needs a `flex-1` class to fill the space between them. Occasion categories used consistently across the gallery filter and order form — Weddings, Events, Birthdays, Holidays, Graduations, Other/Custom — mirroring Jordyn's existing Instagram highlights; don't rename without checking with her. Gallery photos are manually curated/uploaded, never scraped or hotlinked from Instagram — and never substituted with other people's photos (e.g. from Pinterest) as stand-ins, even temporarily, since that misattributes real work as Jordyn's own; the illustrated placeholder cards exist specifically so the site never needs to do that.

**Gallery image infrastructure:** `GalleryItem` (`src/lib/gallery-items.ts`) has an optional `imageSrc`. `GalleryGrid` renders `next/image` (`fill` + responsive `sizes`, auto-optimized/lazy-loaded) when it's set, and the `CakeIllustration` placeholder otherwise — built ahead of Milestone 7 so dropping in Jordyn's real photos is just adding files under `/public` and setting `imageSrc`, no code changes.

**Non-functional priorities:** mobile-first (most traffic arrives from an Instagram bio link on a phone); optimized/responsive gallery images; accessible (contrast, labeled fields, full keyboard nav); customer contact info and uploaded reference images are admin-only, never exposed via a public route or API response. Form inputs use a visible focus ring (`focus:ring-2`), not just a border-color change, which is too weak a signal for keyboard-only users. The Gallery lightbox moves focus to its close button on open and returns focus to the triggering card on close, rather than leaving keyboard focus stranded.
