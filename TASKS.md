...
- [x] [Milestone 4] Added thumbnail previews to the order form's reference-image upload (post-launch enhancement) — selecting files now shows a small preview + filename for each one via a local object URL, plus a client-side warning if more than `MAX_IMAGE_COUNT` are selected (the server-side limit already existed; this just surfaces it earlier). Object URLs are revoked on change/unmount to avoid leaking memory.
