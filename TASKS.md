...
- [ ] Add a delete/decline button on the order detail page, and a "Previous order requests" section (Completed/Declined) below the notify-signups list — added post-launch, not in the original plan. Code is done and tested: `deleteOrder` action, `DeleteOrderButton` (confirm-before-submit, since deletion is irreversible), a one-click "Decline" shortcut, and the dashboard split into active vs. previous orders. **Blocked on you running the new DELETE policy SQL** — tested live with a throwaway admin/order and confirmed the delete silently no-ops until that SQL is applied (RLS correctly has no DELETE policy yet, same "success but no-op" pattern documented elsewhere in this file).
